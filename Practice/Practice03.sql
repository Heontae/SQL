...
직원들의 사번(employee_id), 이름(firt_name), 급여(salary), 부서명(department_name), 현재업무(job_title)
를 사번(employee_id) 오름차순 으로 정렬하세요.
...
employees man
where
emp.manager_id = man.employee_id and
...
job_history jh
where
em.job_id = jh.job_id and
jo.job_id = jh.job_id and
...
where
emp.employee_id = de.manager_id and
de.location_id = lc.location_id and
...
where
emp.manager_id = man.employee_id and
emp.department_id = de.department_id(+)
order by
emp.employee_id asc;
